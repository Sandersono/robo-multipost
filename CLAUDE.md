...
- **Never** give a client (or anyone who must stay scoped to one profile) an org role of `ADMIN`/`SUPERADMIN` — it silently disables per-profile isolation. `ProfileAccessGuard` returns `true` unconditionally for org admins, and `ProfileService.getAccessibleProfiles` skips the membership filter for them, so an admin sees **every** client's calendar, channels and analytics. For an agency whose clients may be competitors, that is the exact failure the profile layer exists to prevent. To let a client publish (not just approve), promote their **profile** role `VIEWER → EDITOR` in Settings → Perfis: writes are allowed and the profile scope holds. Trade-off: `EDITOR` gets the full menu (media, agent, automations) — scoped to their own profile, so not a leak, just more surface than a client needs. A dedicated `CLIENT` role that pairs write access with the `VIEWER` reduced shell is designed but deliberately not built (see [`docs/architecture/future-client-login.md`](docs/architecture/future-client-login.md)); most of what it proposed already exists via `VIEWER` + `@AllowViewer` + the `isViewer` menu gating in `apps/frontend/src/components/layout/top.menu.tsx`.
- **Never** run `gh pr <create|edit|merge|close|ready|review|comment|reopen>` without `--repo Sandersono/robo-multipost` — the fork chain is three levels deep (`gitroomhq/postiz-app` → `maiconramos/robo-multipost` → `Sandersono/robo-multipost`), and `gh` defaults to an upstream we do not own. Enforced by hook `.claude/hooks/gh-pr-fork-guard.sh` (blocks the command with exit 2). Always use `gh pr <subcommand> --repo Sandersono/robo-multipost --base main --head <branch> ...`. Read-only `gh pr view|list|diff|checks|status` are not blocked. **If you re-sync this file from upstream, re-check the target** — upstream ships it pointing at `maiconramos/robo-multipost`, which from here would send every PR to someone else's repo.
